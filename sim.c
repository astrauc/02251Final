...
/* write a byte to the address specified, Needs 4 bytes available */
    ...
    while (1) {
        ...
        switch (decoded.op) {
            ...
            case JAL:
                ...
                /* ~3 to make sure that we increment a program counter by a multiple of 4 */
                core->pc += decoded.imm_j & ~3;
                continue;
            case JALR:
                set_reg(&core->regs, decoded.rd, core->pc + 4);
                core->pc = (get_reg(&core->regs, decoded.rs1) + decoded.imm_i) & ~3;
                continue;
            default:
    ...
    for (int t = 0; t < num_tasks; t++) {
        ...
        while ((entry = readdir(dir)) != NULL) {
            ...
            if (strstr(filename, ".bin") != NULL && strcmp(filename + strlen(filename) - 4, ".bin") == 0) {
                ...
                // Create a binary dump file
                char dump_filepath[1024];
                strncpy(dump_filepath, filename, sizeof(dump_filepath)); 
                dump_filepath[strlen(dump_filepath) - 4] = '\0'; // Remove ".bin"
                strcat(dump_filepath, "_dump.res");

                FILE *dump_file = fopen(dump_filepath, "wb");
                if (dump_file == NULL) {
                    perror("fopen");
                    continue;
                }

                // Write the register values to the dump file
                for (int i = 0; i < 32; i++) {
                    uint32_t reg_value = get_reg(&core.regs, i);
                    fwrite(&reg_value, sizeof(uint32_t), 1, dump_file);
                }
